...
% ylim([-22.55 -22.4]);
grid on;
exportgraphics(gcf,fullfile(outdir,'uc_nonoise.pdf'), 'ContentType','vector');
%% open loop deo
figure;
...
ylim([-26 -15]);
grid on;
exportgraphics(gcf, fullfile(outdir,'uc_openloop_noise.pdf'), 'ContentType','vector');
...
ylim([0.5 2.5]);
grid on;
exportgraphics(gcf,  fullfile(outdir,'il_noise.pdf'), 'ContentType','vector');
...
ylabel('$i_{L}$');
grid on;
exportgraphics(gcf,  fullfile(outdir,'il_openloop_noise.pdf'), 'ContentType','vector');
%% upravljanje
figure;
plot(tSMC, uSMC); hold on 
plot(tLin, uLin); 
...
xlabel('t[s]');
ylabel('u');
legend('BLSMC+I','LIN','FL','Location','northeast');
ylim([0.58 0.7]);
grid on;
exportgraphics(gcf,  fullfile(outdir,'u_noise.pdf'), 'ContentType','vector');
...
grid on;
subplot(2,2,[3 4])
plot(ilOpenLoop, ucOpenLoop); hold on;
xlabel('$x_{1}$');
ylabel('$x_{2}$');
grid on;
exportgraphics(gcf, fullfile(outdir,'noise_openloop.pdf'), 'ContentType','vector');
